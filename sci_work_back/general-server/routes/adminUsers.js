...
router.put("/update", async (req, res) => {
  const user = req.body // Assume all updates are sent in the body
...
router.delete("/delete", async (req, res) => {
    const user = req.body // Assume the user's data is sent in the body
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne({ login: login, password: password, genStatus: 0 })
    if (!user) {
